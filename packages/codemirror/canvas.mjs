...
const getSliderID = (from) => `canvas_${from}`;

export class CanvasWidget extends WidgetType {
  ...
  toDOM() {
    ...
    wrap.className = 'cm-canvas'; // inline-flex items-center
    let slider = wrap.appendChild(document.createElement('div'));
    slider.type = 'range';
    slider.min = this.min;
    slider.max = this.max;
    ...
    slider.addEventListener('input', (e) => {
      ...
      window.postMessage({ type: 'cm-canvas', value: Number(next), id });
    });
    return wrap;
  }
...
export const setCanvasWidgets = StateEffect.define();

export const updateCanvasWidgets = (view, widgets) => {
  view.dispatch({ effects: setCanvasWidgets.of(widgets) });
};

function getSliders(widgetConfigs, view) {
  return widgetConfigs
    .filter((w) => w.type === 'canvas')
    .map(({ from, to, value, min, max, step }) => {
      return Decoration.widget({
        widget: new CanvasWidget(value, min, max, from, to, step, view),
        side: 0,
      }).range(from /* , to */);
    });
}

export const canvasPlugin = ViewPlugin.fromClass(
  class {
 ...
 */
export let canvas = (value) => {
  console.warn('canvas will only work when the transpiler is used... passing value as is');
  return pure(value);
};
// function transpiled from slider = (value, min, max)
export let canvasWithID = (id, value, min, max) => {
  canvasValues[id] = value; // sync state at eval time (code -> state)
  return ref(() => canvasValues[id]); // use state at query time
};
// update state when sliders are moved
if (typeof window !== 'undefined') {
  window.addEventListener('message', (e) => {
    if (e.data.type === 'cm-canvas') {
      if (canvasValues[e.data.id] !== undefined) {
        // update state when slider is moved
        canvasValues[e.data.id] = e.data.value;
      } else {
        console.warn(`slider with id "${e.data.id}" is not registered. Only ${Object.keys(canvasValues)}`);
      }
    }
  });
}
